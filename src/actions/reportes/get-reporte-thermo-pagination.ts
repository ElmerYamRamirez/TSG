...
export const getReporteThermoPagination = async ( page: number,pageSize: number = 15) => {
  try {
    const offset = (page - 1) * pageSize;
    const paramsList = [{ name: "offset", value: offset }, { name: "pageSize", value: pageSize },];

    const query = `
      SELECT * FROM PC_Rep_ReportePrueba
      ORDER BY uniqueId DESC
      OFFSET @offset ROWS FETCH NEXT @pageSize ROWS ONLY;
    `;

    const totalQuery = `SELECT COUNT(*) AS total FROM PC_Rep_ReportePrueba`;
