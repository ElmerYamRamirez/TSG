import { getPrestamosFiltro } from "components/actions";
import { getPrestamosPagination } from "components/actions";
import { getOperadores} from "components/actions";
import UserTable from "components/components/prestamos/prestamos-tabla";
...
import React from "react";
 ...
 export default async function Page({ searchParams }: { searchParams: Promise<{ page?: string; search?: string; desde?: string; hasta?: string }> }) {
  ...
  const pageSize = 15;
    ...
    ? await getPrestamosFiltro(page, pageSize, searchTerm)
    : await getPrestamosPagination(page, pageSize);

    const operadores = await getOperadores() ?? { ok: false, operadores: [] };

  const prestamos = response?.prestamos ?? [];
  const hayMasResultados = prestamos.length === pageSize;

  return (
    <div className="px-2 py-2 max-w-7xl mx-auto">
      <div className="text-center mb-6">
        <h1 className="text-3xl font-bold text-indigo-700">Prestamos</h1>
        <p className="text-sm text-gray-600">Lista de prestamos</p>
      </div>
      ...
      {response?.ok === false ? (
        <p className="text-center text-red-500">Error al cargar los prestamos.</p>
      ) : (
        <>
          {prestamos.length === 0 ? (
            <div className="text-center text-gray-600 mt-6">
              <p>No se encontraron prestamos que coincidan con la búsqueda.</p>
            </div>
          ) : (
            <>
              <UserTable prestamos={prestamos} operadores={operadores.operadores}></UserTable>
